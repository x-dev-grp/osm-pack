...
public class BomService extends BaseServiceImpl<BOM, BOMDto, BOMDto> {
    ...
    public BOMDto getBomById(UUID id) {
        BOM bom = bomRepository.findById(id).orElseThrow(() -> new RuntimeException("BOM non trouvée avec l'id : " + id));
        return convertToDto(bom);
    }
    @Transactional(readOnly = true)
    public List<BOMDto> getBomsBySku(UUID skuId) {
        return bomRepository.findBySkuId(skuId).stream().map(this::convertToDto).collect(Collectors.toList());
    }
    ...
    public BOMDto updateBom(UUID id, BOMDto bomDto) {
        ...
        if (!bom.getSku().getId().equals(bomDto.getSkuId())) {
            SKU newSku = skuRepository.findById(bomDto.getSkuId()).orElseThrow(() -> new RuntimeException("SKU non trouvé avec l'id : " + bomDto.getSkuId()));
            bom.setSku(newSku);
        }
        bom.setVersion(bomDto.getVersion());
        bom.getLines().clear();
