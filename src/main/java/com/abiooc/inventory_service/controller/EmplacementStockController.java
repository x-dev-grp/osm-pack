...
import com.abiooc.inventory_service.dto.EmplacementStockDto;
import com.abiooc.inventory_service.entity.EmplacementStock;

import com.abiooc.inventory_service.service.EmplacementStockService;
...
public class EmplacementStockController extends BaseControllerImpl<EmplacementStock, EmplacementStockDto, EmplacementStockDto> {
    ...
    public ResponseEntity<ApiResponse<EmplacementStock, EmplacementStockDto>> getEmplacementByCode(@PathVariable String code) {
        long startTime = System.currentTimeMillis();
        OSMLogger.logMethodEntry(this.getClass(), "getEmplacementByCode", code);
        try {
            if (code == null || code.trim().isEmpty()) {
                throw new BadRequestException("Emplacement code cannot be empty");
            }
            EmplacementStockDto emplacement = emplacementService.getEmplacementByCode(code);
            log.debug("Successfully fetched emplacement with code: {}", code);
            return ResponseEntity.ok(new ApiResponse<>(true, "Emplacement retrieved successfully", Arrays.asList(emplacement)));
        } catch (NotFoundException e) {
            log.warn("Emplacement not found with code: {}", code);
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new ApiResponse<>(false, e.getMessage(), null));
        } catch (BadRequestException e) {
            log.warn("Bad request for emplacement code: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ApiResponse<>(false, e.getMessage(), null));
        } catch (Exception e) {
            log.error("Error fetching emplacement with code {}: {}", code, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(false, "Error retrieving emplacement: " + e.getMessage(), null));
        } finally {
            OSMLogger.logMethodExit(this.getClass(), "getEmplacementByCode");
            OSMLogger.logPerformance(this.getClass(), "getEmplacementByCode", startTime, System.currentTimeMillis());
        }
    }

    @GetMapping("/type/{type}")
    public ResponseEntity<ApiResponse<EmplacementStock, EmplacementStockDto>> getEmplacementsParType(@PathVariable TypeEmplacement type) {
        long startTime = System.currentTimeMillis();
        OSMLogger.logMethodEntry(this.getClass(), "getEmplacementsParType", type);
        try {
            if (type == null) {
                throw new BadRequestException("Type cannot be null");
            }
            List<EmplacementStockDto> emplacements = emplacementService.getEmplacementsParType(type);
            log.debug("Successfully fetched {} emplacements of type: {}", emplacements.size(), type);
            return ResponseEntity.ok(new ApiResponse<>(true, "Emplacements by type retrieved successfully", emplacements));
        } catch (BadRequestException e) {
            log.warn("Bad request for type: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ApiResponse<>(false, e.getMessage(), null));
        } catch (Exception e) {
            log.error("Error fetching emplacements by type {}: {}", type, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(false, "Error retrieving emplacements by type: " + e.getMessage(), null));
        } finally {
            OSMLogger.logMethodExit(this.getClass(), "getEmplacementsParType");
            OSMLogger.logPerformance(this.getClass(), "getEmplacementsParType", startTime, System.currentTimeMillis());
        }
    }

    @GetMapping("/zone/{zone}")
    public ResponseEntity<ApiResponse<EmplacementStock, EmplacementStockDto>> getEmplacementsParZone(@PathVariable String zone) {
        long startTime = System.currentTimeMillis();
        OSMLogger.logMethodEntry(this.getClass(), "getEmplacementsParZone", zone);
        try {
            if (zone == null || zone.trim().isEmpty()) {
                throw new BadRequestException("Zone cannot be empty");
            }
            List<EmplacementStockDto> emplacements = emplacementService.getEmplacementsParZone(zone);
            log.debug("Successfully fetched {} emplacements in zone: {}", emplacements.size(), zone);
            return ResponseEntity.ok(new ApiResponse<>(true, "Emplacements by zone retrieved successfully", emplacements));
        } catch (BadRequestException e) {
            log.warn("Bad request for zone: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ApiResponse<>(false, e.getMessage(), null));
        } catch (Exception e) {
            log.error("Error fetching emplacements by zone {}: {}", zone, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(false, "Error retrieving emplacements by zone: " + e.getMessage(), null));
        } finally {
            OSMLogger.logMethodExit(this.getClass(), "getEmplacementsParZone");
            OSMLogger.logPerformance(this.getClass(), "getEmplacementsParZone", startTime, System.currentTimeMillis());
        }
    }

    @GetMapping("/disponibles")
    public ResponseEntity<ApiResponse<EmplacementStock, EmplacementStockDto>> getEmplacementsDisponibles() {
        long startTime = System.currentTimeMillis();
        OSMLogger.logMethodEntry(this.getClass(), "getEmplacementsDisponibles");
        try {
            List<EmplacementStockDto> emplacements = emplacementService.getEmplacementsDisponibles();
            log.debug("Successfully fetched {} available emplacements", emplacements.size());
            return ResponseEntity.ok(new ApiResponse<>(true, "Available emplacements retrieved successfully", emplacements));
        } catch (Exception e) {
            log.error("Error fetching available emplacements: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(false, "Error retrieving available emplacements: " + e.getMessage(), null));
        } finally {
            OSMLogger.logMethodExit(this.getClass(), "getEmplacementsDisponibles");
            OSMLogger.logPerformance(this.getClass(), "getEmplacementsDisponibles", startTime, System.currentTimeMillis());
        }
    }

    @GetMapping("/disponibles/zone/{zone}")
    public ResponseEntity<ApiResponse<EmplacementStock, EmplacementStockDto>> getEmplacementsDisponiblesParZone(@PathVariable String zone) {
        long startTime = System.currentTimeMillis();
        OSMLogger.logMethodEntry(this.getClass(), "getEmplacementsDisponiblesParZone", zone);
        try {
            if (zone == null || zone.trim().isEmpty()) {
                throw new BadRequestException("Zone cannot be empty");
            }
            List<EmplacementStockDto> emplacements = emplacementService.getEmplacementsDisponiblesParZone(zone);
            log.debug("Successfully fetched {} available emplacements in zone: {}", emplacements.size(), zone);
            return ResponseEntity.ok(new ApiResponse<>(true, "Available emplacements by zone retrieved successfully", emplacements));
        } catch (BadRequestException e) {
            log.warn("Bad request for zone: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ApiResponse<>(false, e.getMessage(), null));
        } catch (Exception e) {
            log.error("Error fetching available emplacements by zone {}: {}", zone, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(false, "Error retrieving available emplacements by zone: " + e.getMessage(), null));
        } finally {
            OSMLogger.logMethodExit(this.getClass(), "getEmplacementsDisponiblesParZone");
            OSMLogger.logPerformance(this.getClass(), "getEmplacementsDisponiblesParZone", startTime, System.currentTimeMillis());
        }
    }
    ...
    @GetMapping("/temperature-controlee")
    public ResponseEntity<ApiResponse<EmplacementStock, EmplacementStockDto>> getEmplacementsTemperatureControlee() {
        long startTime = System.currentTimeMillis();
        OSMLogger.logMethodEntry(this.getClass(), "getEmplacementsTemperatureControlee");
        try {
            List<EmplacementStockDto> emplacements = emplacementService.getEmplacementsTemperatureControlee();
            log.debug("Successfully fetched {} temperature-controlled emplacements", emplacements.size());
            return ResponseEntity.ok(new ApiResponse<>(true, "Temperature-controlled emplacements retrieved successfully", emplacements));
        } catch (Exception e) {
            log.error("Error fetching temperature-controlled emplacements: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(false, "Error retrieving temperature-controlled emplacements: " + e.getMessage(), null));
        } finally {
            OSMLogger.logMethodExit(this.getClass(), "getEmplacementsTemperatureControlee");
            OSMLogger.logPerformance(this.getClass(), "getEmplacementsTemperatureControlee", startTime, System.currentTimeMillis());
        }
    }
