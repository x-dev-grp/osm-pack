...
import com.abiooc.inventory_service.service.ClientService;
...
public class ClientController extends BaseControllerImpl<Client, ClientDto, ClientDto> {
    ...
    public ResponseEntity<ApiResponse<Client, ClientDto>> getClientByCode(@PathVariable String codeClient) {
        long startTime = System.currentTimeMillis();
        OSMLogger.logMethodEntry(this.getClass(), "getClientByCode", codeClient);
        try {
            if (codeClient == null || codeClient.trim().isEmpty()) {
                throw new BadRequestException("Client code cannot be empty");
            }
            ClientDto client = clientService.getClientByCode(codeClient);
            log.debug("Successfully fetched client with code: {}", codeClient);
            return ResponseEntity.ok(new ApiResponse<>(true, "Client retrieved successfully", Collections.singletonList(client)));
        } catch (NotFoundException e) {
            log.warn("Client not found with code: {}", codeClient);
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new ApiResponse<>(false, e.getMessage(), null));
        } catch (BadRequestException e) {
            log.warn("Bad request for client code: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ApiResponse<>(false, e.getMessage(), null));
        } catch (Exception e) {
            log.error("Error fetching client with code {}: {}", codeClient, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(false, "Error retrieving client: " + e.getMessage(), null));
        } finally {
            OSMLogger.logMethodExit(this.getClass(), "getClientByCode");
            OSMLogger.logPerformance(this.getClass(), "getClientByCode", startTime, System.currentTimeMillis());
        }
    }

    @GetMapping("/email/{email}")
    public ResponseEntity<ApiResponse<Client, ClientDto>> getClientByEmail(@PathVariable String email) {
        long startTime = System.currentTimeMillis();
        OSMLogger.logMethodEntry(this.getClass(), "getClientByEmail", email);
        try {
            if (email == null || email.trim().isEmpty()) {
                throw new BadRequestException("Email cannot be empty");
            }
            ClientDto client = clientService.getClientByEmail(email);
            log.debug("Successfully fetched client with email: {}", email);
            return ResponseEntity.ok(new ApiResponse<>(true, "Client retrieved successfully", Arrays.asList(client)));
        } catch (NotFoundException e) {
            ...
            log.warn("Bad request for email: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ApiResponse<>(false, e.getMessage(), null));
        } catch (Exception e) {
            log.error("Error fetching client with email {}: {}", email, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(false, "Error retrieving client: " + e.getMessage(), null));
        } finally {
            OSMLogger.logMethodExit(this.getClass(), "getClientByEmail");
            OSMLogger.logPerformance(this.getClass(), "getClientByEmail", startTime, System.currentTimeMillis());
        }
    }

    @GetMapping("/recherche")
    public ResponseEntity<ApiResponse<Client, ClientDto>> rechercherClients(@RequestParam String q) {
        long startTime = System.currentTimeMillis();
        OSMLogger.logMethodEntry(this.getClass(), "rechercherClients", q);
        try {
            if (q == null || q.trim().isEmpty()) {
                throw new BadRequestException("Search query cannot be empty");
            }
            List<ClientDto> clients = clientService.rechercherClients(q);
            log.debug("Successfully found {} clients for query: {}", clients.size(), q);
            return ResponseEntity.ok(new ApiResponse<>(true, "Search completed successfully", clients));
        } catch (BadRequestException e) {
            log.warn("Bad search request: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ApiResponse<>(false, e.getMessage(), null));
        } catch (Exception e) {
            log.error("Error searching clients with query {}: {}", q, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(false, "Error searching clients: " + e.getMessage(), null));
        } finally {
            OSMLogger.logMethodExit(this.getClass(), "rechercherClients");
            OSMLogger.logPerformance(this.getClass(), "rechercherClients", startTime, System.currentTimeMillis());
        }
    }

    @GetMapping("/private-label")
    public ResponseEntity<ApiResponse<Client, ClientDto>> getClientsPrivateLabel() {
        long startTime = System.currentTimeMillis();
        OSMLogger.logMethodEntry(this.getClass(), "getClientsPrivateLabel");
        try {
            List<ClientDto> clients = clientService.getClientsPrivateLabel();
            log.debug("Successfully fetched {} private label clients", clients.size());
            return ResponseEntity.ok(new ApiResponse<>(true, "Private label clients retrieved successfully", clients));
        } catch (Exception e) {
            log.error("Error fetching private label clients: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(false, "Error retrieving private label clients: " + e.getMessage(), null));
        } finally {
            OSMLogger.logMethodExit(this.getClass(), "getClientsPrivateLabel");
            OSMLogger.logPerformance(this.getClass(), "getClientsPrivateLabel", startTime, System.currentTimeMillis());
        }
    }

    @GetMapping("/standard")
    public ResponseEntity<ApiResponse<Client, ClientDto>> getClientsStandard() {
        long startTime = System.currentTimeMillis();
        OSMLogger.logMethodEntry(this.getClass(), "getClientsStandard");
        try {
            List<ClientDto> clients = clientService.getClientsStandard();
            log.debug("Successfully fetched {} standard clients", clients.size());
            return ResponseEntity.ok(new ApiResponse<>(true, "Standard clients retrieved successfully", clients));
        } catch (Exception e) {
            log.error("Error fetching standard clients: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(false, "Error retrieving standard clients: " + e.getMessage(), null));
        } finally {
            OSMLogger.logMethodExit(this.getClass(), "getClientsStandard");
            OSMLogger.logPerformance(this.getClass(), "getClientsStandard", startTime, System.currentTimeMillis());
        }
    }
